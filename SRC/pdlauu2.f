...
*     .. Local Scalars ..
     ...
     $                   LDA, MYCOL, MYROW, NA, NPCOL, NPROW
      DOUBLE PRECISION   AII, TEMP
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, INFOG2L, DGEMV, DSCAL, DDOT2
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
            DO 10 NA = N-1, 1, -1
               AII = A( IDIAG )
               ICURR = IDIAG + LDA
               CALL DDOT2(NA, A(ICURR), LDA, A(ICURR), LDA, TEMP)
...
*
            DO 20 NA = 1, N-1
               AII = A( IDIAG )
               ICURR = IDIAG + 1
               CALL DDOT2(N-NA, A(ICURR), 1, A(ICURR), 1, TEMP)
               A(IDIAG) = AII*AII + TEMP
